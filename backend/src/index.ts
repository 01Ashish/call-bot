...
const startServer = async () => {
  try {
    ...
    wss.on("connection", (ws) => {
      ...
      if (!ivrMenu) {
        throw new Error("IVR menu is undefined.");
      }
      ...
      ws.on("message", (data: $TSFixMe) => {
        try {
          ...
          console.error(`Error processing message: ${err?.message ?? "NA"}`);
      ...
      ws.on("error", async (err) => {
        console.error(`socket: ${err.message || "Client received an error."}`);
        // TODO: Implement any necessary cleanup or termination logic
      });
...
const initialize = async () => {
  try {
    ...
    console.error(`Initialization error: ${err?.message ?? "NA"}`);
    deleteActiveCall();
  }
};
